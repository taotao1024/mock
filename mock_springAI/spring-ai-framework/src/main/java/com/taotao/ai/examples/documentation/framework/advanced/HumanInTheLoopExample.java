 ...
 * 1. 配置中断和审批
 * 2. 批准（approve）决策：操作被原样批准并执行，不做任何更改
 * 3. 编辑（edit）决策：工具调用将被修改后执行
 * 4. 拒绝（reject）决策：工具调用被拒绝，并向对话中添加解释
 * 5. 完整示例
 * 6. 实用工具方法
 * <p>
...
public class HumanInTheLoopExample {
	 ...
	 * 为特定工具配置人工审批
	 */
	public void example1_basicConfiguration() throws GraphRunnerException {
		...
		AssistantMessage call = agent.call("执行SQL : select * from a_table");
