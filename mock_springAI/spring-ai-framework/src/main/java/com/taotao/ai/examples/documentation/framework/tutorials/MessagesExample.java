...
/**
 * Messages Tutorial - 完整代码示例展示Messages作为模型交互的基本单元的使用方法
 *
...
public class MessagesExample {
	 ...
	 * 示例7：消息元数据
	 * <p>
	 * 元数据字段的行为因提供商而异 - 有些用于用户识别，有些则忽略它
	 */
	...
	public static void multimodalImageMessage() throws Exception {
		...
		UserMessage userMsg = UserMessage.builder()
				.text("描述这张图片的内容。")
				.media(Media.builder().mimeType(MimeTypeUtils.IMAGE_JPEG).data(new URL("https://http.cat/200"))
						.build()).build();
		Prompt prompt = new Prompt(userMsg);
		ChatResponse call = chatModel.call(prompt);
		System.out.println(call.getResult().getOutput().getText());
	 ...
	 * 示例13：流式和块
	 */
	public static void streamingMessages() throws InterruptedException {
		DashScopeApi dashScopeApi = DashScopeApi.builder()
				.apiKey(System.getenv("AI_DASHSCOPE_API_KEY"))
				.build();
	...
	public static void imageInputFromFile() {
		...
		UserMessage message = UserMessage.builder()
				...
				.media(new Media(
						...
						new ClassPathResource("images/小肥柴_1.jpg")
				))
				.build();
