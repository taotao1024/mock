...
public class MultiAgentExample {
	 ...
	 * <p>
	 * 按顺序执行：Agent按照 subAgents 列表中定义的顺序执行
	 * 状态传递：每个Agent的输出通过 outputKey 存储在状态中，可被后续Agent访问
	 * 消息历史：默认情况下，所有Agent共享消息历史
	...
	public void example4_customMergeStrategy() throws Exception {
		...
		ReactAgent agent3 = ReactAgent.builder()
				.name("agent3")
				.model(chatModel)
				.description("专门写古代诗的AI助手")
				.instruction("你是一个古代的古代诗人，擅长写古代诗。" +
						"用户会给你一个主题，你只需要创作一首古代诗。")
				.outputKey("result3")
				.build();
...
//				.mergeStrategy(new ParallelAgent.DefaultMergeStrategy())
	...
	public void example5_llmRoutingAgent() throws Exception {
		...
		if (result1.isPresent()) {
			...
			state.value("writer_output").ifPresent(System.out::println);
		}
		...
		if (result2.isPresent()) {
			...
			state.value("reviewer_output").ifPresent(System.out::println);
		}
		...
		if (result3.isPresent()) {
			OverAllState state = result3.get();
			state.value("translator_output").ifPresent(System.out::println);
		}
	...
	public void example7_hybridPattern() throws Exception {
		...
		Optional<OverAllState> result = hybridWorkflow.invoke("研究AI技术趋势并生成PDF报告");

		if (result.isPresent()) {
			System.out.println("混合模式示例执行成功");
			result.get().value("research_data").ifPresent(System.out::println);
			result.get().value("pdf_report").ifPresent(System.out::println);
		}
	}
