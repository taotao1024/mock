...
public class HooksExample {
	...
	public static void planning() {
		...
		ReactAgent agent = ReactAgent.builder()
				...
				.model(chatModel)
				.tools(myTool)
				.interceptors(TodoListInterceptor.builder().build()) // 规划
				.build();
	}
	...
	public static void llmToolSelector() {
		...
		ReactAgent agent = ReactAgent.builder()
				...
				.model(chatModel)
				.tools(tool1, tool2)
				.interceptors(ToolSelectionInterceptor.builder().build()) // LLM 工具选择器
				.build();
	}
	...
	public static void llmToolEmulator() {
		...
		ReactAgent agent = ReactAgent.builder()
				...
				.model(chatModel)
				.tools(simulatedTool)
				.interceptors(ToolEmulatorInterceptor.builder().model(chatModel).build()) // LLM 工具模拟器
				.build();
	}
	...
	public static void contextEditing() {
		...
		ReactAgent agent = ReactAgent.builder()
				...
				.model(chatModel)
				.interceptors(ContextEditingInterceptor.builder().trigger(120000).clearAtLeast(60000).build()) // 上下文编辑
				.build();
	}
	...
	public static void demo11() {
		...
		ToolCallback tool1 = createSampleTool();
		ToolCallback tool2 = createSampleTool();
		ToolCallback tool3 = createSampleTool();

		// 按照 1、ToolCallback 2、AgentHook 3、ModelHook 4、Interceptors
		// LoggingInterceptor() 以为先加载 所以在 ToolMonitoringInterceptor() 前面执行 其他先Before123 后After321
		ReactAgent agent = ReactAgent.builder()
				.name("limited_agent")
				.model(chatModel)
				.tools(tool1)
	...
	private static class CustomStopConditionHook extends ModelHook {
