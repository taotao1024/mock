...
public class MemoryExample {
	...
	public static void addShortTermMemory(ChatClient.Builder chatClientBuilder) throws GraphStateException {
		...
		Optional<OverAllState> invoke1 = graph.invoke(Map.of("messages", List.of(
				Map.of("role", "user", "content", "你好！我是 Bob")
		)), config);
		invoke1.ifPresent(state -> {
			state.value("messages").ifPresent(System.out::println);
		});

		// 第二轮对话（使用相同的 threadId）
		Optional<OverAllState> invoke2 = graph.invoke(Map.of("messages", List.of(
				Map.of("role", "user", "content", "我的名字是什么？")
		)), config);
		invoke2.ifPresent(state -> {
			state.value("messages").ifPresent(System.out::println);
		});
	...
	public static void combinedMemoryExample(ChatClient.Builder chatClientBuilder) throws GraphStateException {
		...
		// 第一轮对话（加载偏好并开始对话）
		Optional<OverAllState> resutl = graph.invoke(Map.of(
				"userId", "user_002",
	...
	public static void main(String[] args) {
		...
		try {
			...
			addShortTermMemory(ChatClient.builder(chatModel));
			System.out.println();
			...
			System.out.println("注意: 此示例需要 ChatClient，跳过执行");
			combinedMemoryExample(ChatClient.builder(chatModel));
			System.out.println();
